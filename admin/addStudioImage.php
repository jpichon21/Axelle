...
if (isset($_SESSION['logged_in'])) {
    if (isset($_POST['upload'])) {
        ...
        $target = "../img/contents/sliders/studio/".basename($_FILES['image']['name']);
        $thumb_target = "../img/contents/sliders/studio/thumbs/".basename($_FILES['thumbnail']['name']);

        $image= $_FILES['image']['name'];
        $thumbnail= $_FILES['thumbnail']['name'];
        $title = $_POST['title'];

        if (empty($title) or empty($image) or empty($thumbnail)){
            $error = "Tous les champs sont requis !";
        } else {
            $query = $pdo->prepare('INSERT INTO images_studio (image_alt, image_content, image_thumbnail) VALUES (?, ?, ?)');
            $query->bindValue(1, $title);
            $query->bindValue(2, $image);
            $query->bindValue(3, $thumbnail);
            $query->execute();

            if (move_uploaded_file($_FILES['image']['tmp_name'], $target) and move_uploaded_file($_FILES['thumbnail']['tmp_name'], $thumb_target)) {
...
?>
    <html>
        ...
        <div id="loginPageContainer">
            ...
            <h2>Ajouter une Image Studio</h2>
                <div class="form-group column col-md-10 ">
                    <form action="addStudioImage.php" method="post"  enctype="multipart/form-data">
                        <input type="hidden" name="size" value="1000000">

                        <label for="image">Importer une image</label>
                        <input class="form-input" type="file" name="image"/>

                        <label for="thumbnail">Importer une miniature (format idéal 549*301)</label>
